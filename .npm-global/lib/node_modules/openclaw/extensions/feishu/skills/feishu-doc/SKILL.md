...
**Important:**
1. Always pass `owner_open_id` with the requesting user's `open_id` (from inbound metadata `sender_id`) so the user automatically gets `full_access` permission on the created document.
2. **After creating, immediately use `write` action to add content** - creating an empty document may result in blank content visible to users.

**Example - Create and write in one flow:**
```json
{
  "action": "create",
  "title": "My Document"
